...
def handle_places():
	if request.method == 'POST':
		...
		arr = []
		for place in Place.select():
			arr.append(model_to_dict(place, \
			exclude=[Place.created_at, Place.updated_at]))
...
def handle_place_id(place_id):
	...
	if request.method == 'GET':
		arr.append(model_to_dict(this_place, \
		exclude=[Place.created_at, Place.updated_at]))
...
def handle_place_city_id(state_id, place_id):
	if request.method == 'GET':
		...
		for place in Place.select().where(Place.id == place_id):
			arr.append(model_to_dict(place, exclude=[Place.created_at, \
			Place.updated_at]))
		places = json.dumps(arr)
		return places
	if request.method == 'POST':
