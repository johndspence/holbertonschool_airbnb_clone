...
def handle_amenity():
	if request.method == 'POST':
		...
		arr = []
		for place_book in Amenity.select():
			arr.append(model_to_dict(place_book, \
			exclude=[Amenity.created_at, Amenity.updated_at]))
...
def handle_amenity_id(amenity_id):
	...
	if request.method == 'GET':
		arr.append(model_to_dict(this_amenity, \
		exclude=[Amenity.created_at, Amenity.updated_at]))
...
def handle_place_id_amenity(place_id):
	arr = []
	for placeamenity in \
	PlaceAmenities.select().where(PlaceAmenities.id == place_id):
		if str(placeamenity_id) == str(placeamenity.id):
			this_placeamenity = placeamenity
	if request.method == 'GET':
		arr.append(model_to_dict(this_placeamenity, \
		exclude=[PlaceAmenities.created_at, PlaceAmenities.updated_at]))
		return json.dumps(arr)
